...
<form class="main-form" id="author-form" method="post">
       ...
       <select name="auth-category" id="auth-category" multiple="multiple" size="10" class="required">
        <?php
        $database = new Database;
        $result = $database->find('category', '*');
        while ($row = mysqli_fetch_array($result)) {
            echo '<option id="select-'.$row["id"].'" value="'.$row["name"].'">'.$row["name"].'</option>';
        }
        ?>
        </select>
